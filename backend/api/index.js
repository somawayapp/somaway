...
// ✅ CORS Preflight Handler (critical for POSTs with credentials)
app.options('*', cors());

// 🔁 Middleware to clean up featured/unlisted posts
const unfeatureCleanerMiddleware = async (req, res, next) => {
  try {
    ...
    console.log(`[Middleware] Unlisted ${unlisted.modifiedCount} outdated posts`);
  } catch (err) {
