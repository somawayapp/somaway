...
import G3cycleModel from "../../models/Cycles/G3cycle.model.js";
...
const callbackURL = process.env.MPESA_CALLBACK_BASE_URL + "/g3/callback";
...
// --- Cycle Configuration ---
const MAX_PARTICIPANTS = 1000; //  participants/Ksh



async function getCurrentCycle() {
  let cycle = await G3cycleModel.findOne();
  if (!cycle) {
    cycle = await G3cycleModel.create({ number: 1 }); // first cycle starts at 1
  }
  return cycle;
}
...
async function queryStkStatus(checkoutRequestID) {
  ...
  try {
    ...
    const entry = await G3entryModel.findOne({ transactionId: checkoutRequestID });
...
router.post("/stk-push", async (req, res) => {
  ...
  try {
    const totalParticipants = await G3entryModel.countDocuments({
      status: "Completed",
...
const cycleDoc = await getCurrentCycle();
 ...
 if (totalParticipants >= MAX_PARTICIPANTS) {
  return res.status(403).json({
    ...
    const existingEntry = await G3entryModel.findOne({
    ...
    if (existingEntry) {
     ...
     if (existingEntry.status === "Completed") {
        ...
        await G3entryModel.deleteOne({ _id: existingEntry._id });
      }
    } else {
  ...
  try {
    ...
    newEntry = await G3entryModel.create({ // Assign to newEntry here
      name: encrypt(name),
      phone: encrypt(phone),
      amount: amount,
      phoneNumberHash: phoneNumberHash,
    ...
    if (stkRes.data.ResponseCode === "0") {
      ...
      await G3entryModel.deleteOne({ _id: newEntry._id });
...
// Assuming G3entryModel is your Mongoose model for M-Pesa transactions
