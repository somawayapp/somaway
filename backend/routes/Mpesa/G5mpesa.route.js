...
const callbackURL = `${process.env.MPESA_CALLBACK_BASE_URL}/g5/callback`;
...
// --- Cycle Configuration ---
const MAX_PARTICIPANTS = 100000; //  participants/Ksh



async function getCurrentCycle() {
  let cycle = await G5cycleModel.findOne();
  if (!cycle) {
    cycle = await G5cycleModel.create({ number: 1 }); // first cycle starts at 1
  }
  return cycle;
}
...
const cycleDoc = await getCurrentCycle();
 ...
 if (totalParticipants >= MAX_PARTICIPANTS) {
  return res.status(403).json({
    ...
    if (existingEntry) {
     ...
     if (existingEntry.status === "Completed") {
        ...
        await G5entryModel.deleteOne({ _id: existingEntry._id });
      }
    } else {
  ...
  try {
    ...
    newEntry = await G5entryModel.create({ // Assign to newEntry here
      name: encrypt(name),
      phone: encrypt(phone),
      amount: amount,
      phoneNumberHash: phoneNumberHash,
    ...
    if (stkRes.data.ResponseCode === "0") {
      ...
      await G5entryModel.deleteOne({ _id: newEntry._id });
...
// Assuming G5entryModel is your Mongoose model for M-Pesa transactions
