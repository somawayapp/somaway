...
import G2cycleModel from "../../models/Cycles/G2cycle.model.js"; // <<< Import the Cycle Model
...
async function fetchSummaryData(cycleNumber) {
  const totalGoalAmount = 100; // Define your overall monetary goal here

  // 1. Total amount collected so far (ONLY for "Completed" transactions) for the specific cycle
  const aggregation = await G2entryModel.aggregate([
  ...
  const paymentsForCycle = await G2entryModel.find(
  ...
  const players = await G2entryModel.find({ status: "Completed", cycle: cycleNumber }, { name: 1, phone: 1 })
    .sort({ createdAt: -1 })
    .limit(1000)
    .lean();
...
router.get("/", async (req, res) => {
  try {
    ...
    const currentCycle = await G2cycleModel.findOne().sort({ number: -1 }).lean();
