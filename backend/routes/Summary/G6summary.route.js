...
import G6cycleModel from "../../models/Cycles/G6cycle.model.js"; // <<< Import the Cycle Model
...
async function fetchSummaryData(cycleNumber) {
  const totalGoalAmount = 1000000; // Define your overall monetary goal here

  // 1. Total amount collected so far (ONLY for "Completed" transactions) for the specific cycle
  const aggregation = await G6entryModel.aggregate([
  ...
  const paymentsForCycle = await G6entryModel.find(
  ...
  const players = await G6entryModel.find({ status: "Completed", cycle: cycleNumber }, { name: 1, phone: 1 })
    .sort({ createdAt: -1 })
    .limit(1000)
    .lean();
...
router.get("/", async (req, res) => {
  try {
    ...
    const currentCycle = await G6cycleModel.findOne().sort({ number: -1 }).lean();
