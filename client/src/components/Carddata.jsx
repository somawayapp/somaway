...
export default function BettingChances() {
  ...
  useEffect(() => {
    const fetchCycleStatus = async () => {
      try {
        const res = await fetch("https://shilingiapi.vercel.app/mpesa/g2/cycle-status");
  ...
  useEffect(() => {
    ...
    if (transactionDetails && !["Completed", "Failed", "Cancelled", "Expired", "Query_Failed_Internal"].includes(transactionDetails.status)) {
      ...
      const checkDbStatus = async () => {
        try {
          const res = await fetch("https://shilingiapi.vercel.app/mpesa/g2/get-status", { // NEW ENDPOINT
            method: "POST",
...
👉\n\n Shilingi yaweza kupa mamili. Visit:\nhttps://shilingi.co.ke ✨`;
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("https://shilingiapi.vercel.app/mpesa/g2/stk-push", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
