...
const Sidebar2 = () => {
  ...
  return (
    <div className="w-full md:pr-[25%] px-4 md:px-0 md:pl-[13%] pb-9 md:py-5 overflow-y-auto  text-white flex flex-col items-center gap-4">
      {/* Gauge */}
     {/* Gauge */}
  ...
  >
    {searchLoading ? "Searching..." : "Search"}
      ...
      {searchResults.length > 0 && (
        <motion.div
          className="w-full  text-center h-[fit-content] border border-[#1b1f1c]  max-h-[40vh]  overflow-y-auto"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      {searchResults.length === 0 && ( // Only show the main list if no search results are active
          ...
          <motion.div
          className="w-full text-center h-[fit-content] max-h-[40vh] p-3 bg-gradient-to-br from-[#070707ff] to-[#111] border border-[#1b1f1c] overflow-y-auto "
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.2, duration: 0.5 }}
        >
          ...
          {players.map((player, idx) => (
              <motion.li
                key={idx}
                className="text-[#f2f2f2] mb-1 hover:text-[#ffd700] transition"
